...
function gameTime() {
    _gameTimeNum--;
    if (_gameTimeNum <= 0) {
        GameTimeLayer.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;别吃了,时间到了！';
        gameOver();
...
function showGameScoreLayer() {
    ...
    document.getElementById('GameScoreLayer-bast').innerHTML = '最多吃掉的Guirl个数&nbsp;&nbsp;' + bast;
    l.style.display = 'block';
}
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 49) return 'More practice please';
    if (score <= 99) return '可以哎';
    if (score <= 149) return 'NB';
    if (score <= 199) return '太TMNB了';
    return '你是人吗?';
}
